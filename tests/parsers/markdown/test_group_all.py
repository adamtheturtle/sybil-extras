...
def test_combine_text_empty_examples() -> None:
    """Test that combine_text handles empty examples list correctly.

    This is defensive code that ensures the method works even if called
    with no examples (though in normal operation, finalize returns
    early).
    """
    state = _GroupAllState()
    result = state.combine_text(pad_groups=True)
